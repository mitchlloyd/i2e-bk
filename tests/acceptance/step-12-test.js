...
test("The now-playing component is empty when no song is playing", function(assert) {
  visit('/album/1');

  andThen(function() {
    assert.ok(exists('.now-playing .now-playing-empty'), "The now-playing component is empty when no song is playing");
  });
});

test("Playing a song should update the now-playing template", function(assert) {
  visit('/album/1');
  click('td.song-track:first .play:contains(▶)');

  andThen(function() {
    assert.ok(exists('.now-playing img.now-playing-artwork[src="images/the-morning-after.jpg"]'), "The current song's artwork is showing");
    assert.ok(exists('.now-playing div.now-playing-body .now-playing-name:contains(A Walk)'), "The current song's name is showing");
    assert.ok(exists('.now-playing div.now-playing-body .now-playing-artist:contains(GOLDHOUSE)'), "The current song's artist is showing");
  });
});

test("The player's currentTime starts at 0", function(assert) {
  assert.equal(player.get('currentTime'), 0, "The player's currentTime starts at 0");
});

test("The player's currentTime updates as a song plays showing seconds as integers", function(assert) {
  assert.expect(2);

  let song = Ember.Object.create({
  ...
  waitFor(player, 'currentTime', function() {
    assert.ok(player.get('currentTime') > 0, "currentTime is increasing");
    assert.ok(player.get('currentTime') % 1 === 0, "currentTime is an Integer");
  });
});

test("As a song plays the current time in the now-playing template is updated", function(assert) {
  visit('/album/1');
  click('td.song-track:first .play:contains(▶)');

  andThen(function() {
    assert.ok(contentOf('.now-playing .duration').match(/\d\:\d\d/), "The current time is shown");
  });
});
